...
#  Initial Copyright Ac 2002-2005 Mike Mintz <mikemintz@gmail.com>
#  Modifications Copyright Ac 2007 Als <Als@exploit.in>
#  Modifications Copyright Ac 2007 dimichxp <dimichxp@gmail.com>
#  Parts of code Copyright Ac Boris Kotov <admin@avoozl.ru>
...
BOT_VER = {'rev': 85, 'botver': {'name': 'Talisman+Neutron+Alice', 'ver': 'v3 (svn rev %s) [antiflood],(planb mod)', 'os': 'pythonCE, windows mobile'}}
################################################################################
...
def call_command_handlers(command, type, source, parameters, callee):
   ...
   if COMMAND_HANDLERS.has_key(command):
      if has_access(source, real_access, source[1]):
         ...
         reply(type, source, u'yeah, right')
...
def timeElapsed(time):
   ...
   if time>3600: rep = u'%d hour(s) %s' % (hours, rep)
   if time>86400: rep = u'%d day(s) %s' % (days, rep)
   if time>2592000: rep = u'%d month(s) %s' % (months, rep)
   return rep
      ...
      if fromjid.getStripped() in COMMOFF and command in COMMOFF[fromjid.getStripped()]:
         ...
         if fromjid.getStripped() in GROUPCHATS:      
            if GCHCFGS[fromjid.getStripped()]['autoaway']==1:
               ...
               LAST['gch'][fromjid.getStripped()]['thr']=threading.Timer(600,change_bot_status,(fromjid.getStripped(), u'auto away due to being idle '+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))
...
def presenceHnd(con, prs):
   ...
   if groupchat in GROUPCHATS:
      if ptype == 'unavailable':
         ...
         if not jid:
            time.sleep(2)
            msg(groupchat, u'dammit!! you didnt add my jid to admin list and thus i cannot function, i will leave now ,bye.')
            time.sleep(1)
            leave_groupchat(groupchat, u'Absence of the rights of a moderator')        
            return
         else:
