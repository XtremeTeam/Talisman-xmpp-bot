...
def handler_turn_last(type, source, parameters):
	...
	if parameters:
		...
		if turn_msgs[groupchat][jid] is None:
			reply(type,source,u'And you still spoke nothing')
			return
		if turn_msgs[groupchat][jid] == u'turn':
			reply(type,source,u'aha, sure')
			return		
...
register_command_handler(handler_turn_last, 'turn', ['muc','all'], 10, 'Switches an allocation for last message from the user caused by a command.', 'turn', ['turn'])
