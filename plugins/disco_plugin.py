...
def handler_disco_ext(coze, res, type, source, stop, srch, tojid):
	...
	if id in globals()['disco_pending']:
		...
		reply(type, source, u'what you doing...')
		return
	if res:
...
def handler_disco_answ(type,source,stop,disco,srch):
	total=0
	if total==stop:
		reply(type, source, u'In total '+str(len(disco))+u' Items')
		return
	rep,dis,disco = u'i see:\n',[],sortdis(disco)
	for item in disco:
		if len(item)==3:
			total+=1
			if srch:
	...
	if dis:
		if len(disco)!=len(dis):
			dis.append(u'In total '+str(len(disco))+u' Items')
	else:
		rep=u'empty'
...
register_command_handler(handler_disco, 'disco', ['muc','info','all'], 10, 'Shows results of the review of services for specified JID.\nAlso it is possible to request the review on unit (node). Format of inquiry jid#node.\nВторой или третий (If the terminator кол-ва also is given) Parameter - Search. Searches for the set word in JID and the description of an element диско. If a search word to set a name of conference up to the name of a server (For example qwerty@), That will show a place of this conference in the general rating.\nIn the general chat can give max 50 results, without the instruction кол-ва - 10.\n In приват can give max 250, Without the instruction кол-ва 50.', 'disco <server> <number of results> <a search line>', ['disco jabber.aq','disco conference.jabber.aq 5','disco conference.jabber.aq qwerty','disco conference.jabber.aq 5 qwerty','disco conference.jabber.aq qwerty@', 'disco jabber.aq#services'])
