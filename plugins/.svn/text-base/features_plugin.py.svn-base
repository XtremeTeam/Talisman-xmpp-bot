...
def handler_features_answ(coze, res, type, source, nick):
	...
	if id in globals()['features_pending']:
		...
		print 'oooops...'
		reply(type,source, u'id не совпал...')
	if not res:
	...
	if res.getType()=='error':
		...
		if feat:
			if nick:
				answ=u'клиент '+nick+u' поддерживает следующие стандарты:\n'
			else:
				answ=u'твой клиент поддерживает следующие стандарты:\n'
			feat='\n'.join(feat)
			if type == 'public':
				reply(type,source,u'ушли')
			reply('private',source,answ+feat)
		else:
			reply('private',source,u'глючит клиент')
			
register_command_handler(handler_features_get, 'фичи', ['мук','инфо','все'], 10, 'Показывает, какие XEP\'ы Jabber\'a подерживает твой клиент или клиент определённого юзера или жида.', 'фичи <ник/жид>', ['фичи','фичи guy','фичи guy@jabber.aq/Home','фичи guys@conference.jabber.aq/guy'])