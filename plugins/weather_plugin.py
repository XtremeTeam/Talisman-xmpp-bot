...
def handler_weather_weathercode(type, source, parameters):
	if not parameters:
		...
		return
	if len(parameters)<=2:
		reply(type, source, u'Any chicken-feed...')
		return
	results = ''
...
register_command_handler(handler_weather_weathercode, 'weathercode', ['info','all'], 10, 'Shows a code of city for viewing weather', 'weathercode <city>', ['weathercode orel'])
