...
def macrolist_handler(type, source, parameters):
	rep,dsbll,dsblg,glist,llist=u'MACROLIST:',[],[],[],[]
	if MACROS.macrolist[source[1]]:
		...
		rep += u'\nLOCAL\n'+', '.join(llist)
		if dsbll:
			rep+=u'\n\nThe following local macros are disconnected in this conference:\n'+', '.join(dsbll)
	else:
	...
	rep += u'\nGLOBAL\n'+', '.join(glist)
	if dsblg:
		rep+=u'\n\nThe following global macros are disconnected in this conference:\n'+', '.join(dsblg)
	if type=='public':
