...
def handler_invite_start(type, source, parameters):
	...
	if reason:
		...
		inv.setTagData('reason', u'invited by '+source[2])
	msg.addChild(node=x)
#	print unicode(msg)
	...
	if res:
		...
		for p in [x.getTag('decline') for x in res.getTags('x')]:
			if p!= None:
				...
				if reason:
					reply(type, source, u'The user does not want to come here: '+reason)
				else:
					reply(type, source, u'The user does not want to come here')
"""					
					
register_command_handler(handler_invite_start, 'invite', ['muc','all'], 11, 'invites the user to conference.', 'invite [nick/JID] [reason]', ['invite guy','invite guy@jabber.aq','invite guy@jabber.aq please join'])
