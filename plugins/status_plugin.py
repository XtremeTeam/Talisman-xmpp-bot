...
def handler_status(type, source, parameters):
	if parameters:
		if GROUPCHATS.has_key(source[1]) and GROUPCHATS[source[1]].has_key(parameters):
			...
			if stmsg:
				reply(type,source, parameters+u' is now '+status+u' ('+stmsg+u')')
			else:
				reply(type,source, parameters+u' is now '+status)
		else:
			reply(type,source, u'where is he? :-O')
	else:
		if GROUPCHATS.has_key(source[1]) and GROUPCHATS[source[1]].has_key(source[2]):
			...
			if stmsg:
				reply(type,source, u'you are now '+status+u' ('+stmsg+u')')
			else:
				reply(type,source, u'you are now '+status)
...
register_command_handler(handler_status, 'status', ['info','muc','all'], 0, 'Shows status and status report (if am) of certain user or itself.', 'status <user>', ['status', 'status Pily'])
