...
def dns_query(query):
	try:
		...
		try:
			...
			return u'I did not find something :('
	else:
		try:
...
register_command_handler(handler_dns_dns, 'dns', ['info','all'], 10, 'Shows the answer from DNS for the certain host or IP addresses.', 'dns <host/IP>', ['dns jabber.aq', 'dns 127.0.0.1'])
