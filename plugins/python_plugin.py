...
def handler_python_calc(type, source, parameters):
	...
	if re.sub('([' + string.digits +']|[\+\-\/\*\^\.])','',parameters).strip() == '':
		...
		return_value = u'teach me how to do it :)'
	else:
		return_value = u'something is wrong'
	reply(type, source, return_value)

register_command_handler(handler_python_eval, 'eval', ['superadmin','all'], 100, 'calculates and displays the specified python expression.', 'eval <Expression>', ['eval 1+1'])
register_command_handler(handler_python_exec, 'exec', ['superadmin','all'], 100, 'performs a python expression.', 'exec <Expression>', ['eval pass'])
register_command_handler(handler_python_sh, 'sh', ['superadmin','all'], 100, 'Executes a shell a command.', 'sh <Command>', ['sh ls'])
