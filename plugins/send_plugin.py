...
def handler_send_save(ltype, source, parameters):
   ...
   if GROUPCHATS.has_key(groupchat):
      ...
      fromnick=source[2]+u' Š¾Š. '+source[1]+u' Š2 '+date+u' (UTC) Has asked me to transfer you the following:\n\n'
      if len(args)>=2:
         nick = args[0]
         body = ' '.join(args[1:])
         if nick == 'botadmin':
            reply(ltype, source, u'I shall transfer')
         ...
         if get_bot_nick(groupchat) != nick:
            ...
            if nick in nicks and GROUPCHATS[groupchat][nick]['ishere']==1:
               reply(ltype, source, u'Here sits')
            else:
