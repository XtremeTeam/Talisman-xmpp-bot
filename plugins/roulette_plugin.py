...
def handler_roulette_one(type, source, parameters):
	...
	if not user_level(source,groupchat)>=15:
		if GROUPCHATS.has_key(groupchat):
			if nick:
				...
				if random.randrange(0,2) == 0:
					order_kick(source[1], nick, u'BAAAANGG!!!')
				else:
					rep = u'CLICK!'
		else:
		...
		if rep:
			...
			msg(source[1],  u'/me SHOT!! '+nick)
	else:
		reply(type, source, u'i cant shoot moder!! :(')
	
#def handler_roulette_many(type, source, parameters):
	
	

register_command_handler(handler_roulette_one, 'rr', ['fun','muc','all'], 0, 'Old kind Russian roulette.', 'rr', ['rr'])
